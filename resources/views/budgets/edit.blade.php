...
@section('content')
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        <div class="px-4 py-5 sm:px-6">
            <h3 class="text-lg leading-6 font-medium text-gray-900">Bütçe Düzenle</h3>
        </div>
            ...
            <form method="POST" action="{{ route('budgets.update', $budget->id) }}">
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input type="number" name="amount" id="amount" step="0.01" value="{{ $budget->amount }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                        @error('amount')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        <input type="month" name="month" id="month" value="{{ \Carbon\Carbon::parse($budget->month)->format('Y-m') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                        @error('month')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
